"""Train thesis-aligned H_v2 regressors with resumable Colab outputs.
按论文协议训练 H_v2 回归模型，并支持 Colab 断点续跑。
"""
...
def rebuild_global_outputs(output_dir: Path) -> None:
    """Rebuild global Hv2 tables and report. / 重建全局 Hv2 汇总表和报告。"""
    ...
    for feature_set in FEATURE_FILES:
        ...
        if metadata is not None:
            ...
            lines.append(f"- Runtime versions: scikit-learn={package_versions.get('scikit-learn')}, xgboost={package_versions.get('xgboost')}, lightgbm={package_versions.get('lightgbm')}")
    ...
    lines.extend(
        [
            "## Leakage Guard",
            "",
            "- Excluded from model inputs: `SEQN`, `H_v1`, `H_v2`, `R`, `R_v2`, `H_grade`, `H_grade_quantile`, `available_risk_dimensions`, `age_group`, and all `r_*` columns.",
        ]
    )

    (reports_dir / "hv2_regression_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    """Train H_v2 regressors and persist artifacts. / 训练 H_v2 回归模型并保存产物。"""
    args = parse_args()
    ...
    for model_name in requested_models:
        ...
        try:
            ...
            holdout_pred = pd.Series(holdout_pipeline.predict(X_test), name="H_v2_pred")
            ...
            prediction_frame = make_prediction_frame(meta_test, y_test, holdout_pred, true_column="H_v2_true", pred_column="H_v2_pred")
